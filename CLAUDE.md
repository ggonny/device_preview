...
- `frameProfiles` — 기기 프레임(하드웨어) 프로필. 키: `family`, 베젤 `top/right/bottom/left`, `ring`(금속 테 두께, 나머지 베젤은 검은 글래스), `radius`(바디 코너), `glassRadius`(글래스 베젤 코너 — 생략 시 바디 − ring 동심, 실측값이 다르면 지정), `screenRadius`(디스플레이 코너), `statusBarHeight / statusBarHeightLandscape`(OS 상태바 창 높이 — 생략 시 safeArea.top; 컷아웃 인셋과 상태바 높이가 다른 기기만 지정), `cutout{type,width,height,top|bottom,align,offset}`(세로 위치는 `top` 또는 `bottom` = 아래 가장자리 거리; `dual-hole` 은 `lens/ring/lensGap/flash/flashGap` 으로 [플래시][렌즈][렌즈] 상자 크기가 정해짐), `display.mask:'holes'`(디스플레이 마스크에서 dual-hole 의 렌즈·플래시 원을 뺌), `safeArea` / `safeAreaLandscape`, `homeIndicator`, `homeIndicatorWidth`, `homeIndicatorCenter`(세로 기준 중심 x, 하단 컷아웃 옆에 놓일 때만), `controls{homeButton,speaker,camera,sensor}`, `display.split`(듀얼스크린 전용, 현재 미사용), `hingeBody`(힌지 커버: `{type:'spine',side,width,inset,radius}` | `{type:'seam',size}`), `statusBar`(상태바 프로필 오버라이드), **`source`**(근거 메타데이터).
  ...
  - `family` 값: `iphone-modern | iphone-classic | galaxy-bar | fold | flip | duo | ipad-classic | ipad-modern | galaxy-tab | neutral`(`pixel` 은 2026-09-22 Pixel 프리셋 제거와 함께 삭제). CSS 는 `data-family` 로만 마감(링 색·글래스 색·측면 키)을 분기한다.
- `statusBarProfiles` — OS 상태바(`ios | android | oneui | generic | duo`): 시간 문자열, 아이콘 순서, `cellularStyle: bars(iOS 4막대) | wedge(Pixel 부채꼴) | bars-thin(One UI 가는 4막대)`, `batteryStyle: outline | pill`(One UI 7+ 숫자가 든 알약, `pill{width,height,fontSize}`), 배터리 잔량, 좌우 `padding`, `fontSize / fontWeight / letterSpacing`, `iconSize` + 아이콘별 배율 `batteryScale / cellularScale / wifiScale`, `iconGap`, `percentScale`(배터리 % 글자 = 시간 대비), `batteryText`, `fixedSize`(가로에서도 크기 유지), `centerY / centerYLandscape`(화면 상단 기준 세로 중심 px — 없으면 밴드 중앙). 프레임 `platform` 으로 고르고 `frameProfiles[*].statusBar` 가 덮어쓰며, `statusBar.profile:'oneui'` 처럼 베이스 프로필을 지정할 수 있다. 모든 값은 `updateStatusBar()` 가 CSS 변수(`--status-font/weight/tracking/icon/gap/battery/cellular/wifi/percent/pad-top/pad-bottom, --pill-*, --battery-level`) 로 넘기고 CSS 는 플랫폼별 크기를 하드코딩하지 않는다(가로 축소 규칙·컷아웃별 padding-top 규칙 제거됨).
  ...
  - **표준 Android(`android` 프로필, 공통 기본값)**: AOSP dimens(official) — 시계 14sp · 시스템 아이콘 15dp(가시 ≈12) · 간격 5(2.5sp×2) · 배터리 unified 20.6×12(형상은 외곽선, approximation) · 여백 16/14 approximation. 2026-09-22 Pixel 프리셋 제거 후 직접 쓰는 기기는 없지만 Android 플랫폼 기본값·확장용으로 유지.
...
- `browserProfiles` — 브라우저 UI 밴드 높이(`statusBar / urlBar / toolbar / homeIndicator`, 세로·가로), `urlBarPosition` 기본값, `sideInset:'safe-area'`(가로 letterbox). **`ios-chrome`(2026-09-22 photo-measured, 사용자 제공 iPhone 17 Pro 실기기 Chrome 캡처 `renders/user-chrome/ref.png` 941×2048 = 402×874pt, 2.341px/pt)**: 세로 urlBar **50**(pill 36 = 상 4 / 하 10, 좌우 여백 10, 완전 둥근 평면 회색, URL 14pt 가운데, 선행 페이지 정보 19 · 후행 공유 16×18) · toolbar **44** + 홈 인디케이터 영역 34 에 같은 배경 · 버튼 5개 [← → +(원 Ø36) 탭 카운터 24×24 …] 36 슬롯 `space-between` + 좌우 13.4 → 중심 31.4 / 116.2 / 201 / 285.8 / 370.6(실측 점 아이콘 362.9 는 글리프 오프셋으로 판단), y = 툴바 중앙. Safari 식 공유/북마크 대신 Chrome 구성(`.tb_new`, `.browser_share`, 화살표 `.arrow`) 을 `[data-browser="ios-chrome"]` CSS 로 분기. 값은 pt 공통이라 390/430 에서도 같은 여백으로 배치(뷰포트·아일랜드·safe-area 는 프레임 프로필). 가로 44 는 approximation. 키: `none | ios-safari | ios-chrome | android-chrome | samsung-internet | tablet-chrome | android-tablet-chrome | android-tablet-samsung | ipad-safari | ipados-safari | duo-safari`. `none`(2026-09-22) = 브라우저 UI 없음(모든 밴드 0, iframe = 화면 크기), 범용 프리셋 기본값. `android-tablet-*`(`formFactor:'tablet'`) 는 기본 브라우저가 태블릿 프로필인 기기(Galaxy Tab) 에서만 목록에 노출되며, OS 상태바 24·내비 바 24 높이를 프레임이 아니라 이 프로필에서 명시한다(approximation). `ipados-safari`(홈 인디케이터형 iPad) 는 상태바 = 프레임 safeArea.top(28), 탭 바 50(approximation), 홈 인디케이터 밴드 0(iPad Safari 는 콘텐츠가 그 아래까지 펼쳐짐 → safe-area 가이드로만 표시). 구 키 `phone/compact/tablet` 은 `browserAliases` 로 호환. `duo-safari` 는 `layout:'side'`(측면 컨트롤 열, 밴드 전부 0) · `exclusive`(Duo 기본일 때만 목록) · `standardFallback:'ios-safari'`(표준 가로 바 상태에서 밴드 차용, 상태바 밴드 0) · `side{circle,capsule,pitch,gap,clusterGap,bottomMargin,top,topCapsule,bottomCapsule}`. state 의 `uiLayout:{natural,rotated}` 가 자세별로 `side | bars` 를 정한다.
- `devices` — 프리셋. `name, group, subtitle, note, source, frame, browser, dpr, states[]`. **목록 구성(2026-09-22 정리, 객체 키 순서 = 표시 순서)**: `base` 기준 폭(`kind:'generic'`, 제목 옆 hint "반응형 · breakpoint 점검용", 점선 구분 — 실기기 재현이 아닌 breakpoint 프리셋: 최소 모바일 320×568 · Android 20:9 범용 360×800 · 모바일 기본 390×844 · 대형 모바일 430×932 · Tablet 기본 768×1024(`tablet-generic-768`, neutral 프레임, 가로는 회전)). **기준 폭 프리셋의 기본 브라우저는 `none`(브라우저 UI 없음)** — 밴드 전부 0 이라 iframe CSS 뷰포트 = 목록에 표시된 크기(320×568 · 360×800 · 768×1024, 회전 1024×768). 실제 브라우저 UI 는 선택 목록에서 고를 수 있고(none 은 `formFactor:'generic'` 이라 범용 프리셋에서만 노출), 실기기 프리셋은 상태바·홈 인디케이터가 항상 남는 규칙 유지. 2026-09-22 확정(A안): **기준 폭 5종 모두 neutral 프레임 + `none`** — 모바일 기본 390×844(구 "iPhone 기본") · 대형 모바일 430×932 도 노치/아일랜드·safe-area·상태바 없이 iframe = 표시 크기(회전 844×390 / 932×430). `iphone-notch`·`iphone-dynamic` 프레임 프로필(노치 iPhone 12/13/14 · 아일랜드 430)은 실기기 프리셋·재사용을 위해 데이터로 유지(현재 UI 에서 참조 없음, 노치 상태바 실측값 포함). 테스트의 iOS Safari 밴드 검사는 iPhone 18 Pro 로 이관. / `phone` 주요 스마트폰(iPhone SE (3세대) · iPhone 18 Pro · Galaxy S25) / `fold` 폴더블(Z Flip8 · Z Fold8 · iPhone Duo) / `tablet` 태블릿(iPad Air 11 · Tab S11 · Tab S11 Ultra). 제거된 프리셋: Google Pixel 10(프레임 `android-hole`·CSS family `pixel` 도 삭제; 공통 `android` 상태바·`android-chrome` 브라우저 프로필은 유지), iPad 9.7형 [레거시](프레임 `ipad-home`·브라우저 `ipad-safari` 데이터는 남아 있으나 프리셋 없음; 저장값 `tablet-768/tablet-1024` 는 `deviceAliases` 로 Tablet 기본에 매핑, `pixel` 저장값은 기본 기기로 복귀). state 는 `width/height(CSS px), diagonal, physicalWidth/Height, frame/browser 오버라이드, hinge, hingeSize, hingePosition, orientation('portrait'|'landscape' 기본 자세), scaleWidth/Height`. `deviceGroups`, `deviceAliases` 와 함께 관리. `validateDevices()` 가 누락·오타를 콘솔 경고로 알린다(`source` 누락도 경고).
...
### URL 과 프리뷰 표시의 분리 (2026-09-22 — 시작 안내 영역 완전 제거)
- **시작 안내 영역(`.intro_screen`) 은 마크업·CSS·JS 모두 제거됨**(2026-09-21 오버레이 → 2026-09-22 배너 → 같은 날 제거). 남아 있으면 안 되는 것: `#introScreen/.intro_*` 마크업, "Responsive Device Preview" 문구·제목·설명·"URL 입력하기" CTA·"안내 닫기" 버튼, `#icon-link` 심볼(CTA 전용이었음), `.intro_*` / `.preview_stage.is_intro` CSS, `updateIntro()`, `introDismissed`, intro 버튼 핸들러, `getFitArea()` 의 배너 높이 차감. 다시 만들지 않는다.
- 초기 진입 = 상단 URL 입력 영역 아래에 **선택된(저장된/기본 `mobile-390`) 기기 프리뷰가 바로 표시**된다. 기기 셸·비교 카드는 URL 과 무관하게 항상 렌더·표시되며 기기 선택·상태 전환(Fold/Flip)·회전·브라우저·가이드·배율이 모두 URL 없이 동작한다. `.preview_stage` 의 직계 자식은 `#previewMount · #compareGrid · #compareEmpty` 뿐.
- URL 이 없으면 `loadInto(view)` 가 iframe 에 `initialPreviewContent`(srcdoc placeholder) 를 넣고, URL 이 들어오면 같은 iframe 의 `src` 로 교체한다. **`clearUrl()`**(주소창을 비우고 제출 · 입력창 × 지우기 `search` 이벤트) 은 `currentUrl` 을 비우고 모든 view 를 placeholder 로 되돌리며 프레임·기기 선택·브라우저는 그대로 둔다(`viewportLabUrl` 삭제, 새로고침·사이트 열기 비활성, 상태 문구 "사이트 주소를 입력해 주세요.").
- iframe 안의 srcdoc placeholder(`initialPreviewContent`, 2026-09-21 재디자인, 이후 미수정) 는 "아직 URL 이 연결되지 않은 웹페이지" 처럼 보이는 옅은 wireframe/skeleton: 흰 배경 위에 헤더(로고 블록 + 메뉴 라인) · hero 라인 1~2개 · 콘텐츠 블록(이미지 rect + 라인) 과 작은 한 줄 안내("URL을 입력해 미리보기를 시작하세요." / 좁으면 "URL을 입력해 주세요."). 색은 #eceff3·#f1f3f6 저대비, 카드·아이콘 없음. 안내 문구는 UI 텍스트라 기기 크기에 비례해 줄이지 않고 **15px 고정(≥700px 16px)** — vmin·% 기반 font-size 금지(프리뷰 배율 0.4~0.6 이 추가로 곱해지므로 CSS px 자체가 작으면 읽을 수 없음). 반응형: ≤340px 폭 메뉴 라인 2개·짧은 문구, ≤280px 높이(Flip 커버) hero 보조 라인·블록 라인 생략, ≥700px 블록 3개, ≥1000px 블록 4개.
- Same-Origin 제한 안내는 다중 비교 바·카드 배지(스크롤 동기화 불가 사유) 와 `사용법.txt` 가 담당한다.
...
- iPhone 12/13/14 노치 프레임 `iphone-notch`(390): 베젤 21(derived), 바디 코너 66(photo), 화면 47(reference), 노치 168×33(derived). 14 Pro Max/15 Plus 아일랜드 프레임 `iphone-dynamic`(430): 베젤 19, 화면 55, 바디 74(derived). — 2026-09-22 부터 두 프레임은 프리셋에서 참조하지 않음(390/430 은 범용 기준 폭). 데이터는 유지. iPhone 18 Pro: `iphone-dynamic-pro` 402×874 / 1206×2622(official), 베젤 15·화면 62·상단 safe-area 62(16 Pro 준용, 18 Pro 치수 미확인).
...
- Google Pixel 10 프리셋은 2026-09-22 목록 정리에서 제거(프레임 `android-hole`·family `pixel` CSS 삭제). 재추가 시 이전 값: 베젤 22, 코너 64/54, 펀치홀 30@18(photo, Commons "Google Pixel 9 (Obsidian) front.svg" 10px/mm), 상태바 52(derived).
- iPad 9.7 [레거시] 프리셋은 2026-09-22 목록 정리에서 제거(768×1024 는 범용 "Tablet 기본" 프리셋이 대신함, 저장값은 alias 로 매핑). 프레임 `ipad-home`(family `ipad-classic`)·브라우저 `ipad-safari` 데이터는 재추가 대비로 유지(현재 미사용): 베젤 좌우 56 / 상하 112(derived), 홈 버튼 57(approximation), 화면 코너 0, 상태바 20, 홈 인디케이터 없음, 기본 브라우저 Safari(iPad). 그대로 유지(2026-09-21 iPad Air 추가 시 미수정).
- **Galaxy Tab S11 / S11 Ultra (2026-09-21 추가, family `galaxy-tab`)**: Samsung Global Newsroom 2025-09-04 사양표 + samsung.com/sec·uk 스펙 official — S11 바디 253.8×165.3×5.5mm · native **2560×1600** · 대각선 278.1mm(둥근 모서리 276.2) / Ultra 바디 326.3×208.5×5.1mm · native **2960×1848** · 369.9mm(367.2) · "narrow 5.2mm bezels"(Ultra, 각주: S11 7.8mm). 해상도 세 층위: native official → rendered 1600×2560 / 1848×2960 derived-orientation → cssViewport **800×1280 / 924×1480 @2 derived**(S11 실기기 미검증 — 동일 해상도·밀도 패널의 전작 Tab S9 / S9·S10 Ultra 가 보고된 1440px.com 레퍼런스를 준용한 inferred 값; 실기기 innerWidth·devicePixelRatio 확인 시 verified 로 승격). Samsung 은 CSS 폭·DPR 미공개.
  ...
  - 태블릿 그룹 순서: iPad Air 11 → Galaxy Tab S11 → Galaxy Tab S11 Ultra.
...
iframe 으로는 UA·실제 DPR·터치/hover/pointer 미디어·`env(safe-area-inset-*)`·`dvh/svh/lvh`·대상 페이지 `<meta viewport>` 처리·가상 키보드·브라우저 자동 숨김을 재현할 수 없다. safe-area 는 빗금 가이드로만 보완한다. **스크롤바(2026-09-22)**: placeholder(srcdoc) 는 문서 안 CSS 로, 같은 출처 문서는 `onFrameLoad → applyMobileScrollbarStyle(view)` 가 `<style id="devicePreviewScrollbarStyle" data-device-preview="scrollbar">`(html/body `scrollbar-width:none` + `::-webkit-scrollbar` 0) 을 문서당 1회 주입해 데스크톱 스크롤바를 숨긴다(overflow 미변경 → 스크롤·동기화 그대로, iframe 크기·CSS 뷰포트·`getSiteViewport()` 불변, 문서 안 clientWidth 만 innerWidth 와 같아짐). 다른 출처는 Same-Origin Policy 로 주입 불가 → Windows 에서 데스크톱 스크롤바(약 17px)가 보이는 것을 제한사항으로 둔다. **iframe 폭 +17px·우측 마스크·뷰포트 변경 같은 우회는 금지.**
